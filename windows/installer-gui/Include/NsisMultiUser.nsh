...
!macro MULTIUSER_INIT_VARS
	; required defines
	!ifndef PRODUCT_NAME | VERSION | ICON
		!error "Should define all variables: PRODUCT_NAME, VERSION, ICON"
	!endif
	...
	Function ${UNINSTALLER_FUNCPREFIX}MultiUser.InstallModePre
		...
		!ifdef UMUI_SYSVERSION
			...
			${if} $PerMachineOptionAvailable = 0 ; install per-machine is not available
				SendMessage $MultiUser.InstallModePage.AllUsersLabel ${WM_SETTEXT} 0 "STR:$0($(MULTIUSER_RUN_AS_ADMIN))" ; only when $PerMachineOptionAvailable = 0, we add that comment to the disabled control itself
		...
		!else
			...
			${if} $PerMachineOptionAvailable = 0 ; install per-machine is not available
				SendMessage $MultiUser.InstallModePage.AllUsers ${WM_SETTEXT} 0 "STR:$0($(MULTIUSER_RUN_AS_ADMIN))" ; only when $PerMachineOptionAvailable = 0, we add that comment to the disabled control itself
				${orif} $CmdLineInstallMode != ""
				EnableWindow $MultiUser.InstallModePage.AllUsers 0 ; start out disabled
			${endif}
...
!macro MULTIUSER_RegistryAddInstallInfo
	...
	WriteRegStr SHCTX "${MULTIUSER_INSTALLMODE_UNINSTALL_REGISTRY_KEY_PATH}$0" "DisplayIcon" "$INSTDIR\${ICON}"
	!ifdef COMPANY_NAME
